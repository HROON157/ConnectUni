...
const UMT_Profile = () => {
  ...
  // Fetch UMT students from Firebase
  const fetchUMTStudents = async () => {
    setLoading(true);
    try {
      // First, get students from the 'students' collection where university is UMT
      ...
      if (studentIds.length > 0) {
        ...
        profilesSnapshot.forEach((doc) => {
          ...
          // Check if this profile belongs to a UMT student
      ...
      console.error("Error fetching UMT students:", error);
    } finally {
      setLoading(false);
    }
  };
                  ...
                  {umtStudents.map((student) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="px-2 py-1.5 xs:px-3 xs:py-2 sm:px-4 sm:py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-md xs:rounded-lg  transition-colors duration-200 font-medium text-xs sm:text-sm font-['Public_Sans'] flex-shrink-0 ml-2 xs:ml-3"
                      >
