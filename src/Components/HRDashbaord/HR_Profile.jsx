...
const HR_Profile = () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          if (!base64.startsWith('data:image/webp') || (base64.length / 1024) > maxSizeKB) {
            format = 'image/jpeg'
            quality = 0.8
          ...
          if (finalSizeKB > maxSizeKB * 1.5) {
            toast.warn(`Image compressed to ${Math.round(finalSizeKB)}KB. Consider using a smaller image for better performance.`)
          }
  ...
  const handleFileUpload = useCallback(async (event, type) => {
    ...
    try {
      ...
      const compressedBase64 = await compressImage(file, type === 'profilePic' ? 150 : 100)
          ...
          <div className="mx-auto max-w-full sm:max-w-md md:max-w-lg bg-gray-200 rounded-3xl overflow-hidden shadow-xl backdrop-blur-lg border border-gray-200/50">
            ...
            <div className="px-6 pt-8 pb-6">
              <div className="text-center">
                ...
                <h1 className="text-2xl font-bold text-gray-800 mb-2">{profileData.name || 'HR User'}</h1>
                <p className="text-gray-900 text-sm mb-1">{profileData.title}</p>
            ...
            {profileData.linkedin && (
              <div className="px-6 py-4 border-t border-gray-200/50 mb-4">
                <Link 
                  to={profileData.linkedin.startsWith('http') ? profileData.linkedin : `https://${profileData.linkedin}`}
                  target="_blank" 
                ...
                >
                  <div className="flex items-center gap-2 p-2 rounded-lg transition-colors">
                    ...
                    <span className="text-sm text-gray-600 hover:text-blue-600 transition-colors">LinkedIn Profile</span>
                  </div>
                </Link>
              </div>
            )}
